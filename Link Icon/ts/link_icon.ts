namespace link_icon {
  export const ICON_ID_PREFIX = 'icon-';

  export const enum IconId {
    _BLANK = '_blank',
    AIM = 'aim',
    APP = 'app',
    ARCHIVE = 'archive',
    CSS = 'css',
    DOCUMENT = 'document',
    EPUB = 'epub',
    EXTENSION = 'extension',
    EXTERNAL = 'external',
    FTP = 'ftp',
    GMAIL = 'gmail',
    IMAGE = 'image',
    INSECURE = 'insecure',
    IRC = 'irc',
    JAVA = 'java',
    JAVASCRIPT = 'javascript',
    JAVAWS = 'javaws',
    JSON = 'json',
    LOCAL = 'local',
    PRESENTATION = 'presentation',
    QUICKTIME = 'quicktime',
    MAGNET = 'magnet',
    MAIL = 'mail',
    MUSIC = 'music',
    PDF = 'pdf',
    PSD = 'psd',
    REAL = 'real',
    REGISTRY = 'registry',
    SCRIPT = 'script',
    SECURE = 'secure',
    SPREADSHEET = 'spreadsheet',
    TEXT = 'text',
    TORRENT = 'torrent',
    USER_SCRIPT = 'user_script',
    VIDEO = 'video',
  }

  export const enum Protocol {
    AIM = 'aim:',
    FILE = 'file:',
    FTP = 'ftp:',
    HTTP = 'http:',
    HTTPS = 'https:',
    IRC = 'irc:',
    JAVASCRIPT = 'javascript:',
    MAGNET = 'magnet:',
    MAIL_TO = 'mailto:'
  }

  // Remember to also update the code if you add a value here.
  const enum FileExtension {
    _3G2 = '.3g2',
    _3GP = '.3gp',
    _7Z = '.7z',
    AAC = '.aac',
    AIF = '.aif',
    APP = '.app',
    ASF = '.asf',
    ASX = '.asx',
    AVI = '.avi',
    BAT = '.bat',
    BMP = '.bmp',
    CAF = '.caf',
    COM = '.com',
    CPR = '.cpr',
    CRX = '.crx',
    CSS = '.css',
    DIVX = '.divx',
    DOC = '.doc',
    DOCX = '.docx',
    DVR_MS = '.dvr-ms',
    EPUB = '.epub',
    EXE = '.exe',
    F4V = '.f4v',
    FBR = '.fbr',
    FLAC = '.flac',
    FLV = '.flv',
    GIF = '.gif',
    GZ = '.gz',
    IFF = '.iff',
    JAR = '.jar',
    JAVA = '.java',
    JNLP = '.jnlp',
    JPEG = '.jpeg',
    JPG = '.jpg',
    JS = '.js',
    JSON = '.json',
    LOG = '.log',
    M3U = '.m3u',
    MID = '.mid',
    MIDI = '.midi',
    MOV = '.mov',
    MP3 = '.mp3',
    MP4 = '.mp4',
    MPA = '.mpa',
    MPEG = '.mpeg',
    MPG = '.mpg',
    MTS = '.mts',
    OGG = '.ogg',
    OGM = '.ogm',
    PDF = '.pdf',
    PIF = '.pif',
    PNG = '.png',
    PPT = '.ppt',
    PPTX = '.pptx',
    PSD = '.psd',
    QT = '.qt',
    RA = '.ra',
    RAR = '.rar',
    RCPROJECT = '.rcproject',
    REG = '.reg',
    RM = '.rm',
    RMVB = '.rmvb',
    RNS = '.rns',
    RTF = '.rtf',
    SCM = '.scm',
    SFX = '.sfx',
    SIB = '.sib',
    SIT = '.sit',
    SITX = '.sitx',
    SMIL = '.smil',
    SND = '.snd',
    SVG = '.svg',
    SWF = '.swf',
    TGZ = '.tgz',
    TIF = '.tif',
    TORRENT = '.torrent',
    TS = '.ts',
    TXT = '.txt',
    USER_JS = '.user.js',
    VOB = '.vob',
    VRO = '.vro',
    WAV = '.wav',
    WMA = '.wma',
    WMV = '.wmv',
    WPD = '.wpd',
    WPS = '.wps',
    XLS = '.xls',
    XLSX = '.xlsx',
    XVID = '.xvid',
    ZIP = '.zip',
    ZIPX = '.zipx'
  }

  // Icon: used in LinkIcon
  class Icon {
    constructor(
        readonly id: IconId, readonly name: string,
        readonly description: string, readonly priority: Number,
        readonly settingsOrder: Number, readonly imageBase64: string,
        readonly matches:
            (location: Location, anchor: HTMLAnchorElement, url: URL|undefined,
             urlExtension: string) => boolean) {}

    private iconIdsToDeactivate?: Array<IconId>;

    setIconIdsToDeactivate(iconIdsToDeactivate: Array<IconId>): Icon {
      this.iconIdsToDeactivate = iconIdsToDeactivate;
      return this;
    }

    isEnabled(settings: Record<string, any>): boolean {
      if (settings == undefined) {
        // No settings? We consider everything is enabled.
        return true;
      }

      const value = settings[ICON_ID_PREFIX + this.id];
      return value === undefined || value;
    }

    addAndMaybeDeactivate(iconsToShow: Set<IconId>) {
      iconsToShow.add(this.id);

      if (this.iconIdsToDeactivate != undefined) {
        for (const index in this.iconIdsToDeactivate) {
          iconsToShow.delete(this.iconIdsToDeactivate[index]);
        }
      }
    }
  };

  // Returns a new array where the given icons are ordered between each other
  // using the given icon field retriever to compare them.
  function sortIconsBy(
      icons: MapIterator<Icon>,
      fieldGetterFn: (icon: Icon) => Number): Array<Icon> {
    const sorted: Array<Icon> = [];

    for (const icon of icons) {
      // If no element yet, add first one and loop immediately
      if (sorted.length == 0) {
        sorted.push(icon);
        continue;
      }

      let i;
      for (i = 0; i < sorted.length; i++) {
        if (fieldGetterFn(sorted[i]) > fieldGetterFn(icon)) {
          // Icon in the priority list has a greater priority than
          // the icon we want to insert. So, insert it.
          sorted.splice(i, 0, icon);
          break;
        }
      }

      if (sorted[i] != icon) {
        // Not inserted in priority list, add it to the end
        sorted.push(icon);
      }
    }

    return sorted;
  }

  // LinkIcon: contains all icons with pre-calculated ordered icon arrays.
  export class LinkIcon {
    constructor() {
      this.icons = buildIconsMap();
      this.iconsByPriority = sortIconsBy(this.icons.values(), (icon: Icon) => {
        return icon.priority;
      });
      this.iconsBySettingsOrder =
          sortIconsBy(this.icons.values(), (icon: Icon) => {
            return icon.settingsOrder;
          });
    }

    readonly iconsByPriority: Array<Icon>;
    readonly iconsBySettingsOrder: Array<Icon>;
    private readonly icons: Map<IconId, Icon>;
  };

  //
  // Classes declaration finished, now on to the code
  //

  function addIconToIconsMap(icon: Icon, iconsMap: Map<IconId, Icon>) {
    iconsMap.set(icon.id, icon);
  }

  function buildIconsMap(): Map<IconId, Icon> {
    const iconsMap = new Map<IconId, Icon>();

    // Target
    addIconToIconsMap(
        new Icon(
            IconId._BLANK, 'New tab/window',
            'Opens a new tab or window depending on your browser settings.',
            /*priority=*/ 0, /*settingsOrder=*/ 0, /*imageBase64=*/
            '',
            (_location: Location, anchor: HTMLAnchorElement,
             _url: URL|undefined, _urlExtension: string) => {
              return anchor.target == '_blank';
            }),
        iconsMap);


    // Protocols
    addIconToIconsMap(
        new Icon(
            IconId.AIM, 'AIM',
            'An AIM resource (new chat window, new buddy, ...).',
            /*priority=*/ 10, /*settingsOrder=*/ 12, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             url: URL|undefined, _urlExtension: string) => {
              if (!url) {
                return false;
              }
              return url.protocol == Protocol.AIM;
            })
            .setIconIdsToDeactivate([IconId.EXTERNAL]),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.FTP, 'FTP', 'A resource on a FTP server.',
            /*priority=*/ 10, /*settingsOrder=*/ 13, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             url: URL|undefined, _urlExtension: string) => {
              if (!url) {
                return false;
              }
              return url.protocol == Protocol.FTP;
            })
            .setIconIdsToDeactivate([IconId.EXTERNAL]),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.IRC, 'IRC', 'An IRC server (and channel).',
            /*priority=*/ 10, /*settingsOrder=*/ 11, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             url: URL|undefined, _urlExtension: string) => {
              if (!url) {
                return false;
              }
              return url.protocol == Protocol.IRC;
            })
            .setIconIdsToDeactivate([IconId.EXTERNAL]),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.LOCAL, 'Local file',
            'A file located on the local host (your machine).',
            /*priority=*/ 10, /*settingsOrder=*/ 10, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             url: URL|undefined, _urlExtension: string) => {
              if (!url) {
                return false;
              }
              return url.protocol == Protocol.FILE;
            })
            .setIconIdsToDeactivate([IconId.EXTERNAL]),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.MAIL, 'Mail',
            'Creates a new message in your mail client and opens it.',
            /*priority=*/ 10, /*settingsOrder=*/ 14, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             url: URL|undefined, _urlExtension: string) => {
              if (!url) {
                return false;
              }
              return url.protocol == Protocol.MAIL_TO;
            })
            .setIconIdsToDeactivate([IconId.EXTERNAL]),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.MAGNET, 'Magnet', 'A magnet link.',
            /*priority=*/ 10, /*settingsOrder=*/ 15, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             url: URL|undefined, _urlExtension: string) => {
              if (!url) {
                return false;
              }
              return url.protocol == Protocol.MAGNET;
            })
            .setIconIdsToDeactivate([IconId.EXTERNAL]),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.SCRIPT, 'Script',
            'Will trigger a javascript action when you click.',
            /*priority=*/ 10, /*settingsOrder=*/ 16, /*imageBase64=*/
            '',
            (_location: Location, anchor: HTMLAnchorElement, url: URL|undefined,
             _urlExtension: string) => {
              if (!url) {
                return false;
              }
              return url.protocol == Protocol.JAVASCRIPT ||
                  (anchor.onclick != null && anchor.onclick.length > 0);
            })
            .setIconIdsToDeactivate([IconId.EXTERNAL]),
        iconsMap);

    // Hosts
    addIconToIconsMap(
        new Icon(
            IconId.GMAIL, 'Gmail', 'Redirects to Google Mail.',
            /*priority=*/ 10, /*settingsOrder=*/ 40, /*imageBase64=*/
            '',
            (location: Location, _anchor: HTMLAnchorElement, url: URL|undefined,
             _urlExtension: string) => {
              if (location.hostname == 'mail.google.com') {
                // Don't display this on Gmail, duh
                return false;
              }
              if (!url) {
                return false;
              }
              return url.hostname == 'mail.google.com' ||
                  url.hostname == 'www.gmail.com';
            })
            .setIconIdsToDeactivate([IconId.EXTERNAL]),
        iconsMap);

    // File types
    addIconToIconsMap(
        new Icon(
            IconId.APP, 'Application',
            'An application file (app, bat, exe, ...).',
            /*priority=*/ 0, /*settingsOrder=*/ 50, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.APP ||
                  urlExtension == FileExtension.BAT ||
                  urlExtension == FileExtension.COM ||
                  urlExtension == FileExtension.EXE ||
                  urlExtension == FileExtension.PIF;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.ARCHIVE, 'Archive',
            'A compressed file or an archive (7z, rar, zip, ...).',
            /*priority=*/ 0, /*settingsOrder=*/ 64, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.GZ ||
                  urlExtension == FileExtension.JAR ||
                  urlExtension == FileExtension.RAR ||
                  urlExtension == FileExtension._7Z ||
                  urlExtension == FileExtension.SFX ||
                  urlExtension == FileExtension.SIT ||
                  urlExtension == FileExtension.SITX ||
                  urlExtension == FileExtension.TGZ ||
                  urlExtension == FileExtension.ZIP ||
                  urlExtension == FileExtension.ZIPX;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.DOCUMENT, 'Document',
            'A rich-text document (doc, rtf, ...).',
            /*priority=*/ 0, /*settingsOrder=*/ 52, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.DOC ||
                  urlExtension == FileExtension.DOCX ||
                  urlExtension == FileExtension.RTF ||
                  urlExtension == FileExtension.WPD ||
                  urlExtension == FileExtension.WPS;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.EXTENSION, 'Google Chrome Extension',
            'This will install a new extension on your browser!',
            /*priority=*/ 0, /*settingsOrder=*/ 63, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.CRX;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.IMAGE, 'Image',
            'You should see an image by clicking on this link.',
            /*priority=*/ 0, /*settingsOrder=*/ 53, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.BMP ||
                  urlExtension == FileExtension.GIF ||
                  urlExtension == FileExtension.JPEG ||
                  urlExtension == FileExtension.JPG ||
                  urlExtension == FileExtension.PNG ||
                  urlExtension == FileExtension.SVG ||
                  urlExtension == FileExtension.TIF;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.PSD, 'Photoshop Image',
            'A Photoshop image. You will need a special software to open these.',
            /*priority=*/ 0, /*settingsOrder=*/ 53, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.PSD;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.JAVA, 'Java Source File',
            'You should see an image by clicking on this link.',
            /*priority=*/ 0, /*settingsOrder=*/ 66, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.JAVA;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.JAVAWS, 'Java Applet/Archive',
            'A Java Web Start applet or a Java archive (may be an application).',
            /*priority=*/ 0, /*settingsOrder=*/ 67, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.JAR ||
                  urlExtension == FileExtension.JNLP;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.JAVASCRIPT, 'Javascript File', 'A Javascript source file.',
            /*priority=*/ 0, /*settingsOrder=*/ 54, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.JS;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.JSON, 'JSON Stream', 'A stream using the JSON format.',
            /*priority=*/ 0, /*settingsOrder=*/ 54, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.JSON;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.CSS, 'CSS File', 'A CSS source file.',
            /*priority=*/ 0, /*settingsOrder=*/ 54, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.CSS;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.MUSIC, 'Music', 'A mucic file (mp3, ogg, wav, ...).',
            /*priority=*/ 0, /*settingsOrder=*/ 51, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.AAC ||
                  urlExtension == FileExtension.AIF ||
                  urlExtension == FileExtension.CAF ||
                  urlExtension == FileExtension.CPR ||
                  urlExtension == FileExtension.FLAC ||
                  urlExtension == FileExtension.IFF ||
                  urlExtension == FileExtension.M3U ||
                  urlExtension == FileExtension.MID ||
                  urlExtension == FileExtension.MIDI ||
                  urlExtension == FileExtension.MP3 ||
                  urlExtension == FileExtension.MPA ||
                  urlExtension == FileExtension.OGG ||
                  urlExtension == FileExtension.RA ||
                  urlExtension == FileExtension.RNS ||
                  urlExtension == FileExtension.SIB ||
                  urlExtension == FileExtension.SND ||
                  urlExtension == FileExtension.WAV ||
                  urlExtension == FileExtension.WMA;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.PDF, 'PDF',
            'Will open your PDF reader application and open the document.',
            /*priority=*/ 0, /*settingsOrder=*/ 55, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.PDF;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.EPUB, 'ePUB', 'e-book file using the ePUB standard format.',
            /*priority=*/ 0, /*settingsOrder=*/ 56, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.EPUB;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.PRESENTATION, 'Presentation',
            'A file used for presentations (ppt, ...).',
            /*priority=*/ 0, /*settingsOrder=*/ 52, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.PPT ||
                  urlExtension == FileExtension.PPTX;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.QUICKTIME, 'Quicktime media',
            'A file readable with Apple Quicktime (c).',
            /*priority=*/ 0, /*settingsOrder=*/ 57, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.MOV ||
                  urlExtension == FileExtension.QT;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.REAL, 'Real media', 'A file readable with RealPlayer (c).',
            /*priority=*/ 0, /*settingsOrder=*/ 58, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.RM ||
                  urlExtension == FileExtension.RMVB;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.REGISTRY, 'Registry',
            'A registry file (be careful with those files!).',
            /*priority=*/ 0, /*settingsOrder=*/ 59, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.REG;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.SPREADSHEET, 'Spreadsheet', 'A spreadsheet (xls, ...).',
            /*priority=*/ 0, /*settingsOrder=*/ 62, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.XLS ||
                  urlExtension == FileExtension.XLSX;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.TORRENT, 'Torrent', 'A spreadsheet (xls, ...).',
            /*priority=*/ 0, /*settingsOrder=*/ 60, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.TORRENT;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.TEXT, 'Text files', 'A raw text file (log, txt, ...).',
            /*priority=*/ 0, /*settingsOrder=*/ 61, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension.LOG ||
                  urlExtension == FileExtension.TXT;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.USER_SCRIPT, 'User script', 'A user script.',
            /*priority=*/ 5, /*settingsOrder=*/ 68, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             url: URL|undefined, _urlExtension: string) => {
              if (!url) {
                return false;
              }
              let urlLastTwoExtensions = url.pathname.substring(
                  url.pathname.substring(0, url.pathname.lastIndexOf('.'))
                      .lastIndexOf('.'));
              return urlLastTwoExtensions == FileExtension.USER_JS;
            })
            .setIconIdsToDeactivate([IconId.JAVASCRIPT]),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.VIDEO, 'Video', 'A video file (avi, divx, mpg, ogm, ...).',
            /*priority=*/ 0, /*settingsOrder=*/ 65, /*imageBase64=*/
            '',
            (_location: Location, _anchor: HTMLAnchorElement,
             _url: URL|undefined, urlExtension: string) => {
              return urlExtension == FileExtension._3G2 ||
                  urlExtension == FileExtension._3GP ||
                  urlExtension == FileExtension.ASF ||
                  urlExtension == FileExtension.ASX ||
                  urlExtension == FileExtension.AVI ||
                  urlExtension == FileExtension.DIVX ||
                  urlExtension == FileExtension.DVR_MS ||
                  urlExtension == FileExtension.F4V ||
                  urlExtension == FileExtension.FBR ||
                  urlExtension == FileExtension.FLV ||
                  urlExtension == FileExtension.MP4 ||
                  urlExtension == FileExtension.MPEG ||
                  urlExtension == FileExtension.MPG ||
                  urlExtension == FileExtension.MTS ||
                  urlExtension == FileExtension.OGM ||
                  urlExtension == FileExtension.RCPROJECT ||
                  urlExtension == FileExtension.SCM ||
                  urlExtension == FileExtension.SMIL ||
                  urlExtension == FileExtension.SWF ||
                  urlExtension == FileExtension.TS ||
                  urlExtension == FileExtension.VOB ||
                  urlExtension == FileExtension.VRO ||
                  urlExtension == FileExtension.WMV ||
                  urlExtension == FileExtension.XVID;
            }),
        iconsMap);

    // Security
    addIconToIconsMap(
        new Icon(
            IconId.SECURE, 'Secure connection',
            'A SSL secured connection will be opened by clicking on this link (only displayed on non secured pages).',
            /*priority=*/ 0, /*settingsOrder=*/ 90, /*imageBase64=*/
            '',
            (location: Location, _anchor: HTMLAnchorElement, url: URL|undefined,
             _urlExtension: string) => {
              if (!url) {
                return false;
              }
              // We're changing protocol
              if (location.protocol != url.protocol) {
                // If we're on an "http"-based page
                if (location.protocol == Protocol.HTTP) {
                  if (url.protocol == Protocol.HTTPS) {
                    // Going to "https"-based (secure) page
                    return true;
                  }
                }
              }
              return false;
            }),
        iconsMap);
    addIconToIconsMap(
        new Icon(
            IconId.INSECURE, 'Insecure connection',
            'You will return to a non secured connection by clicking on this link (only displayed on secured pages).',
            /*priority=*/ 0, /*settingsOrder=*/ 91, /*imageBase64=*/
            '',
            (location: Location, _anchor: HTMLAnchorElement, url: URL|undefined,
             _urlExtension: string) => {
              if (!url) {
                return false;
              }
              // We're changing protocol
              if (location.protocol != url.protocol) {
                // If we're on an "http"-based page
                if (location.protocol == Protocol.HTTPS) {
                  if (url.protocol == Protocol.HTTP) {
                    // Going to "http"-based (insecure) page
                    return true;
                  }
                }
              }
              return false;
            }),
        iconsMap);

    // External
    addIconToIconsMap(
        new Icon(
            IconId.EXTERNAL, 'External link',
            'You will reach an external domain by clicking on this link. Note that switching country on the same site does not trigger this icon (for example switching from "ebay.com" to "ebay.co.jp").',
            /*priority=*/ 0, /*settingsOrder=*/ 100, /*imageBase64=*/
            '',
            (location: Location, _anchor: HTMLAnchorElement, url: URL|undefined,
             _urlExtension: string) => {
              if (!url) {
                return false;
              }
              return !tld.isSameRootDomainName(location.host, url.host);
            }),
        iconsMap);

    return iconsMap;
  }

}  // namespace link_icon
